...
void run_proto(int runnum, int fileCount, int min_evt, int max_evt, int gap_point_0=0, int gap_pt2s_0=0, int gap_pt1s_0=0, int gap_hs_0=0, int gap_point_1=0, int gap_pt2s_1=0, int gap_pt1s_1=0, int gap_hs_1=0)
{
	string evtDir = "2cubes";
    ...
    for (int j = runnum; j < runnum + fileCount; j++) {
        TString pt2sfile_name = TString::Format("%sproto2s_%04d_0004_tree2.root", rootfile_dir.c_str(), j);
        TString pt1sfile_name = TString::Format("%sproto1s_%04d_0004_tree2.root", rootfile_dir.c_str(), j);
        TString hsfile_name   = TString::Format("%shodo_%04d_0004_tree2.root", rootfile_dir.c_str(), j);
        /*
        if (j == ) {
          gap_point_0 = ;
          gap_pt2s_0 = ;
        ...
        */
        ...
        for (int evt = 0; evt < less_evt; evt++) {
            ...
            mkdir("2cubes/evtdisplay", S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP | S_IXGRP | S_IROTH | S_IXOTH | S_IXOTH);
            figname1 = TString::Format("2cubes/evtdisplay/hit_%d_%d.pdf", runnum, evt);
            ...
            for (int i = 0; i < 75; i++) {
                ...
                if (i < 50) {
                    ...
                    if ( (i ==  6 || i ==  7 || i ==  8 || i == 11 || i == 12 || i == 13 || i == 16 || i == 17 || 1 == 18) && pe_1s[map[i]] > 29.5)
					{
                        trig_1s = true;
                    }
                }
        ...
        TString  figname3 = TString::Format("2cubes/distance_%d.pdf", runnum);
	...
	string dir;
	dir = evtDir + "/75ch/";
	mkdir (dir.c_str(), S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP | S_IXGRP | S_IROTH | S_IXOTH | S_IXOTH);
    ofstream outputtext5(dir + "array_PE.tsv");
    ...
    for (int x = 0; x < 75; x++) {
		...
		dir = evtDir + "/75ch/run%d_PE_position%d.pdf";
        pdfname1    = TString::Format(dir.c_str(), runnum, x);
...
{
    if (argc <= 4) {
        cerr << "run_proto <runnum> <# of files> <min_evt> <max_evt>" << endl;
    ...
    int min_evt     = atoi(argv[3]);
    int max_evt     = atoi(argv[4]);
    ...
    run_proto(runnum, fileCount, min_evt, max_evt);
