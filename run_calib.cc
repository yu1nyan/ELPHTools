...
// modified by Y.Awataguchi in 2018/11/12 -> no connectionなチャンネルを飛ばすようにした
// modified by Y.Awataguchi in 2018/11/13 -> pedestalフィットの範囲を変更 定数で変更できるように
// modified by Y.Awataguchi in 2018/11/13 -> pedestalフィットの範囲を前半・後半32ch個別に変えられるようにした
// modified by Y.Awataguchi in 2018/11/14 -> pedestalフィット範囲を引数で与えられるように
#include <iostream>
#include <fstream>
#include <sstream>
#include <cstdlib>
...
void run_calib(string type, int nrun, int nsub, int file_count, int low, int high, int pedMaxFH, int pedMaxLH, string imgtype)
{
    const int PedMin = 700;

    ToScintiCh* toScintiCh;
    if (type != "hodo")
    {
        switch (nsub)
        {
            ...
            default: cerr << "Invalid sub run number!!" << endl; return;
        }
    }
    ...
    if (type == "hodo")
    {
        easiroc = EEasiroc::Hodoscope;
    }
    else if (type == "proto1s")
    {
        easiroc = EEasiroc::Scinti1;
    }
    else if (type == "proto2s")
    {
    ...
    mkdir(calibfile_dir.c_str(), 0777);

    // array<bool, 64> is_connected = { };  // 使うチャンネルの定義
    // if (type == "hodo")
    // {
    //     for (int i = 0; i < 64; i++)
    //     {
    ...
    //     }
    // }
    // else if (type == "proto1s" || type == "proto2s")
    // {
    //     for (int i = 1; i <= 15; i++)
    //     {
    //         is_connected[i] = true;
    ...
    {
        TString inputfile_name_t = TString::Format("%s_%04d_%04d_tree2.root", type.c_str(), nrun + n, nsub);
        ...
        struct stat statBuf;
        cout << calibdir_s << endl;
        // if (stat(calibdir_s.c_str(), &statBuf) != 0)
        // {
        //     mkdir(calibdir_s.c_str(), 0777);
        // }
        mkdir(calibdir_s.c_str(), 0777);
        ...
        for (int ch = 0; ch < 64; ch++)
        {
            ...
            // {
            //     h1[ch]->GetXaxis()->SetRange(PedMin, pedMaxHodo);
            // }
            // // proto2s
            // if (pos3 >= 0 && pos2 == string::npos && pos4 == string::npos)
            // {
            //     if (ch <= 31)
            //     {
            //         h1[ch]->GetXaxis()->SetRange(PedMin, pedMaxProto2sFH);
            //     }
            //     else
            //     {
            //         h1[ch]->GetXaxis()->SetRange(PedMin, pedMaxProto2sLH);
            //     }
            // }
            // // proto1s
            // if (pos4 >= 0 && pos3 == string::npos && pos2 == string::npos)
            // {
            //         h1[ch]->GetXaxis()->SetRange(PedMin, pedMaxProto1s);
            // }

            if (ch <= 31)
            {
                h1[ch]->GetXaxis()->SetRange(PedMin, pedMaxFH);
            }
            else if (ch >= 32)
            {
                h1[ch]->GetXaxis()->SetRange(PedMin, pedMaxLH);
            }
            ...
            cout << "ch" << ch << ": " << ped_const[ch] << " " << binmax1[ch] << endl;
            h1[ch]->Fit("gaus_ped", "Q+", "", binmax1[ch] - 12, binmax1[ch] + 12);
            ...
            // 1p.e. peak fit
            if (pos3 == string::npos && pos4 == string::npos && ch >= 32)
            {
            ...
            h1[ch]->Fit("gaus_1pe", "Q+", "", binmax2[ch] - 8, binmax2[ch] + 13);
            ...
            if (gain[ch] < 25 || gain[ch] > 50)
            {
                ...
                if (pos3 == string::npos && pos4 == string::npos)
                {
                    ...
                    h1[ch]->Fit("gaus_1pe", "Q+", "", mean_ped[ch] + 33, mean_ped[ch] + 55);
                }
                else if ((pos2 == string::npos && pos4 == string::npos) ||
                         (pos3 == string::npos && pos2 == string::npos))
                {
                    f2->SetParameters(pe_const[ch], mean_ped[ch] + 30, 7.);
                    h1[ch]->Fit("gaus_1pe", "Q+", "", mean_ped[ch] + 23, mean_ped[ch] + 45);
                }
                mean_1pe[ch] = f2->GetParameter(1);
                sigma_1pe[ch] = f2->GetParameter(2);
                chi2_ndf_1pe[ch] = f2->GetChisquare() / f2->GetNDF();
            ...
            name2 = TString::Format("%s/%s_ch%d.%s", calibdir_s.c_str(), calibfile_name.c_str(), ch, imgtype.c_str());
        ...
        c2->Modified();
        TString namec2;
        namec2 = TString::Format("%s/%s_gain_dist.%s", calibdir_s.c_str(), calibfile_name.c_str(), imgtype.c_str());
        c2->SaveAs(namec2);
        ...
        c3->Modified();
        TString namec3;
        namec3 = TString::Format("%s/%s_pedestal_dist.%s", calibdir_s.c_str(), calibfile_name.c_str(), imgtype.c_str());
        c3->SaveAs(namec3);
        ...
        c4->Modified();
        TString namec4;
        namec4 = TString::Format("%s/%s_1pe_dist.%s", calibdir_s.c_str(), calibfile_name.c_str(), imgtype.c_str());
        c4->SaveAs(namec4);
        ...
        c5->Modified();
        TString namec5;
        namec5 = TString::Format("%s/%s_pedsigma_dist.%s", calibdir_s.c_str(), calibfile_name.c_str(), imgtype.c_str());
        c5->SaveAs(namec5);
        ...
        c6->Modified();
        TString namec6;
        namec6 = TString::Format("%s/%s_1pesigma_dist.%s", calibdir_s.c_str(), calibfile_name.c_str(), imgtype.c_str());
        c6->SaveAs(namec6);
        ...
        c7->Modified();
        TString namec7;
        namec7 = TString::Format("%s/%s_pedchi2_dist.%s", calibdir_s.c_str(), calibfile_name.c_str(), imgtype.c_str());
        c7->SaveAs(namec7);
...
int main(int argc, char** argv)
{
    if (argc != 10)
    {
        cerr << "run_calib <detector type> <nrun> <nsub> <# of files> <pedmax+LOW> <pedmax+HIGH> <pedRangeFH> <pedRangeLH>  <png, eps, pdf>" << endl;
    ...
    // int    hodo       = atoi(argv[7]);
    // int    proto1s    = atoi(argv[8]);
    // int    proto2sFH  = atoi(argv[9]);
    // int    proto2sLH  = atoi(argv[10]);
    int pedMaxFH = atoi(argv[7]);
    int pedMaxLH = atoi(argv[8]);

    string imgtype = argv[argc - 1];

    run_calib(type, nrun, nsub, file_count, low, high, pedMaxFH, pedMaxLH, imgtype);
