...
void run_1cube(int &runnum, int &min_evt, int &max_evt, int &file_count)
{
    ...
    for (int j = runnum; j < runnum + file_count; j++) {
        TString ptfile_name = TString::Format("%sproto_%04d_0003_tree2.root", rootfile_dir.c_str(), j);
        TString hsfile_name = TString::Format("%shodo_%04d_0003_tree2.root", rootfile_dir.c_str(), j);
        /*
          int gap_point_0 = 0;
          int gap_pt_0 = 0;
          int gap_hs_0 = 0;
        ...
        */
        ...
        for (int y = 0; y < 16; y++) {
            for (int x = 0; x < 16; x++) {
                ...
                int omit_lowx2  = x;
                int omit_highx2 = 15 - x;
                int omit_lowy2  = y;
                int omit_highy2 = 15 - y;
                ...
                for (int evt = 0; evt < less_evt; evt++) {
                    ...
                    figname1 = TString::Format("1cube/evtdisplay/hit_%d_%d.pdf", runnum, evt);
                    ...
                    for (int i = 0; i < 3; i++) {
                        ...
                        if (i == 0) {
                            hCubeX->SetBinContent(1, 1, pe_p[map[i]]);
                            if (pe_p[map[i]] > 9.5) {
                                hHitRate_X->Fill(1, 1);
                                trig_X = true;
                                if (good_event) {
                                    hPE_cell_all[x][y]->Fill(pe_p[map[i]]);
                                    hPE_cell_x[x][y]->
                                    hPE_X->Fill(pe_p[map[i]]);
                                }
                            }
                        }
                        if (i == 1) {
                            hCubeY->SetBinContent(1, 1, pe_p[map[i]]);
                            if (pe_p[map[i]] > 9.5) {
                                hHitRate_Y->Fill(1, 1);
                                trig_Y = true;
                                if (good_event) {
                                    hPE_Y->Fill(pe_p[map[i]]);
                                }
                            }
                        }
                        if (i == 2) {
                            hCubeZ->SetBinContent(1, 1, pe_p[map[i]]);
                            if (pe_p[map[i]] > 9.5) {
                                hHitRate_Z->Fill(1, 1);
                                trig_Z = true;
                                if (good_event) {
                                    hPE_Z->Fill(pe_p[map[i]]);
                                }
                            }
                        }
                    }
                ...
                TString  figname3 = TString::Format("1cube/distance_%d.pdf", runnum);
    ...
    for (int x = 0; x < 75; x++) {
        ...
        pdfname1    = TString::Format("1cube/75ch/run%d_PE_position%d.pdf", runnum, x);
    ...
    c3->SaveAs("1cube/hitRate_allRun.pdf");
    ...
    c4->SaveAs("1cube/hodoscope_HitMultiplicity_allRun.pdf");
    ...
    c5->SaveAs("1cube/proto_PE_allRun.pdf");
}

int main(int argc, char **argv)
{
    if (argc != 5) {
        cerr << "run_1cube <1st run#> <# of files> <min_evt> <max_evt>" <<
            endl;
    ...
    run_1cube(runnum, min_evt, max_evt, file_count);
