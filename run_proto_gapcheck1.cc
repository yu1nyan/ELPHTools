...
void run_proto(int &runnum, int &nsub, int &file_count, int &min_evt, int &max_evt) {
  ...
  //make gapcheck histogram
  TString gapcheck_name_z = TString::Format("gapcheck_z%04d+%d_%04d.root2",runnum,file_count-1,nsub);
  TH2F* gapcheck_z = new TH2F(gapcheck_name_z.Data(), "gapcheck_z", 5, 0.5, 5.5, 5, 0.5, 5.5);
  gapcheck_z->SetMinimum(binmin);
  gapcheck_z->SetMaximum(binmax+20);
  TString gapcheck_name_x = TString::Format("gapcheck_x%04d+%d_%04d.root2",runnum,file_count-1,nsub);
  TH2F* gapcheck_x = new TH2F(gapcheck_name_x.Data(), "gapcheck_x", 5, 0.5, 5.5, 5, 0.5, 5.5);
  gapcheck_x->SetMinimum(binmin);
  gapcheck_x->SetMaximum(binmax+20);
 ...
 cout << "Select X2:" << "[" << omit_lowx2+1 << ", " <<16-omit_highx2 <<"]" << endl;
  ...
  for(int evt = 0; evt < less_evt; evt++) {
    ...
    */
    ...
    bool gapcheck_count=false;
    double max_hsx=0;
    double max_hsy=0;
    int gapcheck_hsx_ch=0;
    int gapcheck_hsy_ch=0;
    gapcheck_count=false;
    for ( int i=0; i<64; i++ ) {
    ...
    if(gapcheck_hsx_ch==11 && gapcheck_hsy_ch==27){
      gapcheck_count=true;
    }
    ...
    double max_proto_z=0;
    int gapcheck_z_ch=0;
    double max_proto_x=0;
    int gapcheck_x_ch=0;
    for ( int i = 0; i < 75; i++ ) {
	...
	pe_2s[map[i]] = double(floor(pe_2s[map[i]]*10)/10);
	  ...
	  if(max_proto_x<pe_2s[map[i]]){
	    max_proto_x=pe_2s[map[i]];
	    gapcheck_x_ch=i;
	  }
	  if (pe_2s[map[i]] > 9.5) {
	...
	if(max_proto_z<pe_1s[map[i]]){
	  max_proto_z=pe_1s[map[i]];
	  gapcheck_z_ch=i;
	}
	if (pe_1s[map[i]] > 39.5) {
    ...
    if(gapcheck_count==true){
      gapcheck_z->Fill(Bij_x[gapcheck_z_ch-50], Bij_y[gapcheck_z_ch-50]);
      gapcheck_x->Fill(Bij_x[gapcheck_x_ch], Bij_y[gapcheck_x_ch]);
    }
...
TString pdfname1;
 gStyle->SetOptFit(1111);
  ...
  TCanvas*c10 = new TCanvas("c10","",1800,600);
  c10->Divide(2,1);
  c10->cd(2);
  gapcheck_z->GetYaxis()->SetTitle();
  gapcheck_z->GetXaxis()->SetTitle();
  gapcheck_z->GetYaxis()->SetTitleSize(0.03);
  gapcheck_z->GetXaxis()->SetTitleSize(0.03);
  gapcheck_z->GetYaxis()->SetLabelSize(0.04);
  gapcheck_z->GetXaxis()->SetLabelSize(0.04);
  gapcheck_z->GetXaxis()->SetNdivisions(5);
  gapcheck_z->GetYaxis()->SetNdivisions(5);
  gapcheck_z->SetMarkerSize(2.0);
  gapcheck_z->Draw("text colz");

  c10->cd(1);
  gapcheck_x->GetYaxis()->SetTitle();
  gapcheck_x->GetXaxis()->SetTitle();
  gapcheck_x->GetYaxis()->SetTitleSize(0.03);
  gapcheck_x->GetXaxis()->SetTitleSize(0.03);
  gapcheck_x->GetYaxis()->SetLabelSize(0.04);
  gapcheck_x->GetXaxis()->SetLabelSize(0.04);
  gapcheck_x->GetXaxis()->SetNdivisions(5);
  gapcheck_x->GetYaxis()->SetNdivisions(5);
  gapcheck_x->SetMarkerSize(2.0);
  gapcheck_x->Draw("text colz");
  TString c10_name = TString::Format("prototype/gapcheck_%04d+%d_%04d.pdf",runnum,file_count-1,nsub);
  c10->SaveAs(c10_name.Data());
}
